...
import { verifyKakaoToken } from '../../lib/loginUtil';

const parameter = {
  type: 'object',
  properties: {
    accessToken: { type: 'string' }, // kakao에서 발급한 authorization code
...
export const handler = async (event: APIGatewayProxyEventV2) => {
  ...
  try {
    // kakao server에서 발급한 accessToken 검증 및 payload 조회
    let userEmail: string, fullName: string;
    try {
      const { email, name } = await verifyKakaoToken(kakaoAccessToken);
      userEmail = email;
      fullName = name;
    } catch (err) {
